...
async fn rpc(context: Ctx, _next: MiddlewareNext<Ctx>) -> MiddlewareResult<Ctx> {
    ...
    let resp = GreeterServer::new(MyGreeter::default()).call(hyper_request).await.unwrap();
...
fn main() {
    ...
    let mut app = App::<HyperRequest, Ctx, MyGreeter>::create(generate_context, MyGreeter::default());
    app.post("/helloworld.Greeter/SayHello", async_middleware!(Ctx, [rpc]));
