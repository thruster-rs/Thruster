...
fn it_should_execute_all_middlware_with_a_given_request() {
  ...
  app.use_middleware("/", async_middleware!(BasicContext, [query_params::query_params]));
...
fn it_should_handle_cookies() {
  ...
  let _ = Runtime::new().unwrap().block_on(async {
    let response = testing::request(&app, "GET", "/test", &vec![("Set-Cookie", "Hello=World; SameSite=Strict")], "").await;
...
fn it_should_match_as_far_as_possible_in_a_subapp() {
  ...
  #[middleware_fn]
  async fn test_fn_2(mut context: BasicContext, _next: MiddlewareNext<BasicContext>) -> BasicContext {
    context.body("-1");
    context
  }
...
fn it_should_execute_all_middlware_with_a_given_request_up_and_down() {
  ...
  #[middleware_fn]
  async fn test_fn_2(mut context: BasicContext, next: MiddlewareNext<BasicContext>) -> BasicContext {
    let existing_body = context.get_body().clone();
  ...
  app.get("/test", async_middleware!(BasicContext, [test_fn_2, test_fn_1]));
...
fn it_should_first_run_use_then_methods() {
  ...
  #[middleware_fn]
  async fn method_agnostic(mut context: BasicContext, next: MiddlewareNext<BasicContext>) -> BasicContext {
...
fn it_should_be_able_to_correctly_handle_not_found_routes() {
  ...
  #[middleware_fn]
  async fn test_404(mut context: BasicContext, _next: MiddlewareNext<BasicContext>) -> BasicContext {
