...
use core::errors::{ErrorSet, ThrusterError as Error};
use core::{map_try, MiddlewareNext, MiddlewareResult, MiddlewareReturnValue};
use lazy_static::*;
...
pub fn get_file(path: &str) -> Result<Bytes, std::io::Error> {
    let path = path.replace("/static", "").replace("..", "");
