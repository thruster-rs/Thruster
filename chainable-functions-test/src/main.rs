...
async fn main() {
    ...
    let f: ChainableFn<_, _, _> = (
        ...
        |c: String, _next: AsyncNext<String, _>| async move {
            format!("10{c}")
        }
    ).into();
...
fn _a(a: i32, next: Next<i32, i32>) -> i32 { (next)(a + 1) }
fn _b(b: i32, next: Next<String, String>) -> i32 { (next)(b.to_string()).parse::<i32>().unwrap() } 
fn _c(c: String, _next: Next<String, String>) -> String { format!("10{c}") }
