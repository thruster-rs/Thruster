...
impl<R: RequestWithParams, T: Context + Send> App<R, T> {
  ...
  #[cfg(not(feature = "hyper_server"))]
  pub fn resolve(&self, request: R, matched_route: MatchedRoute<T>) -> impl Future<Output=Result<T::Response, Error=io::Error>> + Send {
    self._resolve(request, matched_route)
  }
  ...
  // fn _resolve(&self, request: R, matched_route: MatchedRoute<T>) -> impl Future<Output=Result<T::Response, Error=io::Error>> + Send {
  //   use thruster_async_await::resolve;
  ...
  #[cfg(not(feature = "thruster_async_await"))]
  fn _resolve(&self, mut request: R, matched_route: MatchedRoute<T>) -> impl Future<Output=Result<T::Response, Error=io::Error>> + Send {
...
mod tests {
  ...
  fn it_should_execute_all_middlware_with_a_given_request() {
    ...
    fn test_fn_1(mut context: BasicContext, _next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> Box<dyn Future<Output=Result<BasicContext, Error=io::Error>> + Send> {
      context.body("1");
  ...
  fn it_should_correctly_differentiate_wildcards_and_valid_routes() {
    ...
    fn test_fn_404(mut context: BasicContext, _next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> Box<dyn Future<Output=Result<BasicContext, Error=io::Error>> + Send> {
      context.body("2");
      Box::new(future::ok(context))
    };
  ...
  fn it_should_match_as_far_as_possible_in_a_subapp() {
    ...
    fn test_fn_2(mut context: BasicContext, _next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> MiddlewareReturnValue<BasicContext> {
      context.body("-1");
      Box::new(future::ok(context))
    }
  ...
  fn it_should_execute_all_middlware_with_a_given_request_based_on_method() {
    ...
    fn test_fn_2(mut context: BasicContext, _next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> Box<dyn Future<Output=Result<BasicContext, Error=io::Error>> + Send> {
      let existing_body = context.get_body().clone();
  ...
  fn it_should_execute_all_middlware_with_a_given_request_up_and_down() {
    ...
    fn test_fn_2(mut context: BasicContext, next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> Box<dyn Future<Output=Result<BasicContext, Error=io::Error>> + Send> {
      let existing_body = context.get_body().clone();
  ...
  fn it_should_first_run_use_then_methods() {
    ...
    fn method_agnostic(mut context: BasicContext, next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> Box<dyn Future<Output=Result<BasicContext, Error=io::Error>> + Send> {
  ...
  fn it_should_be_able_to_correctly_handle_not_found_routes() {
    ...
    fn test_404(mut context: BasicContext, _next: impl Fn(BasicContext) -> MiddlewareReturnValue<BasicContext>  + Send) -> Box<dyn Future<Output=Result<BasicContext, Error=io::Error>> + Send> {
